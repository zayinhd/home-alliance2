...
export default function CustomerTrackingScreen() {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                const providerIds = activeJobs
                    .map((job: any) => job.service_provider_id)
                    .filter(Boolean);

                const providerLocations =
                    await getLocationsByUserIds(providerIds);
                const index: Record<string, any> = {};
                providerLocations.forEach((row: any) => {
    ...
    return (
        <View className="flex-1 bg-white pt-14 px-4">
            <Text className="text-3xl font-Jost-Bold mb-2">Live Tracking</Text>
            <Text className="text-gray-500 mb-4">
                Tracking {trackedCount} active service provider locations.
            </Text>
            ...
            <FlatList
                ...
                renderItem={({ item }) => {
                    const location = locations[item.service_provider_id];
                    return (
